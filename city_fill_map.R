...
# load_data -----------------------------------------------------------------------------------
data_city <- read_csv("dataset/data_city.csv")
cn <- st_read("dataset/china_city_full_map.json") %>%
  mutate(city_id = str_sub(NAME, 1, 2))
cn %>%
  left_join(data_city,
            by = c("city_id" = "city")) %>%
  ...
  mutate(classify=cut(casenum,
                      breaks = c(0.0, 19.99, 78.99, 159.99,259.99,399.9,100000),
                      labels = c("无", "20 ~ 78",  "79 ~ 159","160 ~ 259","260 ~ 399",
                                 "> 400"),
                      include.lowest = TRUE)) -> map_data_city
...
ggplot(map_data_city) + 
  geom_sf(data=subset(map_data_city,
                      NAME!="国界线"),
          aes(fill=classify), 
  ...
  guides(fill = guide_legend(title = "Population",
                             ...
                             title.hjust = 0.3,
                             nrow = 3)) + 
  labs(title = "cityince Distribution") + 
  #scale_fill_gradient(low = "white",high = "#99000d")+
  scale_fill_manual(values = rev(col_rmy))+
  ...
  annotation_north_arrow(location = "tl", which_north = "False",
                         ...
                         pad_x = unit(0.5, "cm"),
                         pad_y = unit(0.65, "cm"),
                         style = north_arrow_fancy_orienteering(text_family = "enfont"))  -> city_p
city_p

ggsave(filename = "output/city_fig_rmy_color.svg",plot =city_p,width = 12,height = 9,dpi = 800)
